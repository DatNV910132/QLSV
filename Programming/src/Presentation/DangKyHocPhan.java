	...
	private void initialize(String mssv,String name) throws SQLException {
		...
		cbhocky.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				DangkyhocphanController Connectsqldkhp = new DangkyhocphanController();
				ConnectMysql.Connect();
				HocPhanController Connectsql = new HocPhanController();
				ConnectMysql.Connect();
				tableHP.removeAll();
				Connectsqldkhp.LoaddataDKHPtotable(
						Connectsqldkhp.dkhp.getData_DangkyHocphan(mssv, cbhocky.getSelectedItem().toString()), tableHP);
				int rc = tableHP.getRowCount();
				for (int i = 0; i < rc; i++) {
					ResultSet rs2 = Connectsql.hocphan.getData_Hocphan(tableHP.getValueAt(i, 0).toString());
					try {
						if (rs2.next()) {
		...
		SinhVienController sqlSV = new SinhVienController();
		ConnectMysql.Connect();
		ResultSet rs = sqlSV.sinhvien.getData_Sinhvien(mssv);
		...
		btndangky.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				TongTC = 0;
				ResultSet rs1 = Connectsql.hocphan.getData_Hocphan(txtHP.getText());
				String MaHP = null;
				try {
					if (rs1.next()) {
				...
				if (flag) {
					Connectsql.LoaddataHPtotable(Connectsql.hocphan.getData_Hocphan(txtHP.getText()), tableHP, numrow);
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if (numrow == 0) {
					...
					TongTC = 0;
					DangkyhocphanController Connectsql = new DangkyhocphanController();
					ConnectMysql.Connect();
					HocPhanController Connectsqlhp = new HocPhanController();
					ConnectMysql.Connect();
					Connectsql.dkhp.deleteDKHP(tableHP, cbhocky.getSelectedItem().toString(), mssv);
					Connectsql.LoaddataDKHPtotable(
							Connectsql.dkhp.getData_DangkyHocphan(mssv, cbhocky.getSelectedItem().toString()), tableHP);
					int rc = tableHP.getRowCount();
					for (int i = 0; i < rc; i++) {
						ResultSet rs2 = Connectsqlhp.hocphan.getData_Hocphan(tableHP.getValueAt(i, 0).toString());
						try {
							if (rs2.next()) {
								tableHP.setValueAt(rs2.getObject(2), i, 1);
								tableHP.setValueAt(rs2.getObject(3), i, 4);
			...
			public void actionPerformed(ActionEvent e) {
				...
				for (int i = 0; i < rc; i++) {
					if (tableHP.getValueAt(i, 3) == null) {
						Connectsql.dkhp.insertDB_DangkyHP(
				...
				for (int j = 0; j < rcn; j++) {
					ResultSet rs2 = Connectsqlhp.hocphan.getData_Hocphan(tableHP.getValueAt(j, 0).toString());
					try {
						if (rs2.next()) {
